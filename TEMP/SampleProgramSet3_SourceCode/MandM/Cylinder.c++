...
/*
	Matrix4x4 m = Matrix4x4.yRotationDegrees(Math.pi / 2);
	for (int i=0 ; i<=N_POINTS_AROUND_SLICE ; i++) {
		normals[i] = normals[i];
	}
	...
	*/
	...
	int size = nPoints * sizeof(vec3);
	glBufferData(GL_ARRAY_BUFFER, size, coords, GL_STATIC_DRAW);
	glVertexAttribPointer(shaderIF->pvaLoc("mcPosition"), 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(shaderIF->pvaLoc("mcPosition"));

	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
	int size2 = nPoints * sizeof(vec3);
	glBufferData(GL_ARRAY_BUFFER, size2, normals, GL_STATIC_DRAW);
