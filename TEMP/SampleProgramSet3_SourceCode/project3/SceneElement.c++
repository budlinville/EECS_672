...
//#include "../imageutil/ImageReader.h"		//NOTE: NEED TO REINCLUDE

#define ACTUAL_NUM_LIGHTS 3
...
void SceneElement::establishLightingEnvironment()
{
	...
	int actualNumLights = ACTUAL_NUM_LIGHTS;

	for (int i = 0; i < actualNumLights; i++) {
		glUniform4fv(shaderIF->ppuLoc("p_ecLightPosition"), 1, posToGLSL);
		glUniform3fv(shaderIF->ppuLoc("lightStrength"), 1, lightStrength);
		glUniform3fv(shaderIF->ppuLoc("globalAmbient"), 1, globalAmbient);
		glUniform3fv(shaderIF->ppuLoc("ia"), 1, ambientIntensity);
	}
}

void SceneElement::establishMaterial()
{
	glUniform3fv(shaderIF->ppuLoc("ka"), 1, matl.ka);		//ambient lighting
	glUniform3fv(shaderIF->ppuLoc("kd"), 1, matl.kd);		//diffuse lighting
	glUniform3fv(shaderIF->ppuLoc("ks"), 1, matl.ks);		//specular lighting
}
