...
House::House(ShaderIF* sIF, PhongMaterial& matl, float cx, float cy, float cz, float l) : SceneElement(sIF, matl) {
	xmin = cx; xmax = cx + l;
...
void House::defineHouse() {
	...
	***********************************/
	...
	//HOUSE TRIM
	PhongMaterial trimPhong = PhongMaterial(0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 1.0, 1.0);

	trim[0] = new Block(shaderIF, trimPhong, xmax, ymin, zmax, TRIM_DELTA, length, TRIM_DELTA);									//front right vertical trim
	trim[1] = new Block(shaderIF, trimPhong, (xmin - TRIM_DELTA), ymin, zmax, TRIM_DELTA, length, TRIM_DELTA);		//front left vertical trim
	trim[2] = new Block(shaderIF, trimPhong, (xmin - TRIM_DELTA), ymin, zmin - TRIM_DELTA, TRIM_DELTA, length, TRIM_DELTA);		//back left vertical trim
	trim[3] = new Block(shaderIF, trimPhong, xmax, ymin, zmin - TRIM_DELTA, TRIM_DELTA, length, TRIM_DELTA);									//back right vertical trim

	//WINDOW TRIM
	trim[4] = new Block(shaderIF, trimPhong, doorWindowLeft - TRIM_DELTA, windowBottom - TRIM_DELTA, zmax, windowLength + 2 * TRIM_DELTA, TRIM_DELTA, TRIM_DELTA);		//window bottom trim
	trim[5] = new Block(shaderIF, trimPhong, doorWindowLeft - TRIM_DELTA, windowTop, zmax, windowLength + 2 * TRIM_DELTA, TRIM_DELTA, TRIM_DELTA);		//window bottom trim
	trim[6] = new Block(shaderIF, trimPhong, doorWindowLeft - TRIM_DELTA, windowBottom, zmax, TRIM_DELTA, windowLength, TRIM_DELTA);		//window left trim
	trim[7] = new Block(shaderIF, trimPhong, doorWindowRight, windowBottom, zmax, TRIM_DELTA, windowLength, TRIM_DELTA);		//window right trim

	//DOOR TRIM
	trim[8] = new Block(shaderIF, trimPhong, doorWindowLeft - TRIM_DELTA, ymin, zmax, TRIM_DELTA, DOOR_HEIGHT + TRIM_DELTA, TRIM_DELTA);		//left left trim
	trim[9] = new Block(shaderIF, trimPhong, doorWindowRight, ymin, zmax, TRIM_DELTA, DOOR_HEIGHT + TRIM_DELTA, TRIM_DELTA);		//left left trim
	trim[10] = new Block(shaderIF, trimPhong, doorWindowLeft, doorTop, zmax, windowLength, TRIM_DELTA, TRIM_DELTA);		//window bottom trim

	trim[11] = new Block(shaderIF, trimPhong, xmin, yHeight, zmax, length, TRIM_DELTA, TRIM_DELTA);		//top house trim
	trim[12] = new Block(shaderIF, trimPhong, xmin, yHeight, zmin - TRIM_DELTA, length, TRIM_DELTA, TRIM_DELTA);		//top house trim
...
void House::renderHouse() {
	...
	/*************************************
							ROOF - change color
	*************************************/
	matl.kd[0] = 0.4; matl.kd[1] = 0.2; matl.kd[2] = 0.1;
	matl.ka[0] = 0.4; matl.ka[1] = 0.2; matl.ka[2] = 0.1;
	matl.ks[0] = 0.4; matl.ks[1] = 0.2; matl.ks[2] = 0.1;
	//glUniform3fv(shaderIF->ppuLoc("kd"), 1, matl.kd);
	//glUniform3fv(shaderIF->ppuLoc("ka"), 1, matl.ka);
	//glUniform3fv(shaderIF->ppuLoc("ks"), 1, matl.ks);

	//NOTE: NO LONGER USING - POOR PROGRAMMING PRACTICE
