...
Fence::Fence(ShaderIF* sIF, PhongMaterial& matl, float cx, float cy, float cz, float l, float w, float h) : SceneElement(sIF, matl) {
	xmin = cx;
	xmax = cx + l;
	ymin = cy;
	ymax = cy + l;
...
void Fence::defineFence() {
	...
	//PhongMaterial postPhong = PhongMaterial(0.3, 0.3, 0.3, 0.15, 0.15, 0.15, 0.08, 0.08, 0.08, 1.0, 1.0);

	for (int i = 0; i < NUM_POSTS_PER_SIDE; i++) {
		post[index] = new Block(shaderIF, matl, newPost, ymin, zmin, postWidth, postHeight, postWidth);
		newPost += delta;
		index++;
	}
	...
	newPost = zmin;
	for (int i = 0; i < NUM_POSTS_PER_SIDE; i++) {
		post[index] = new Block(shaderIF, matl, (xmax - postWidth), ymin, newPost, postWidth, postHeight, postWidth);
		newPost += delta;
		index++;
	}
	...
	for (int i = 0; i < NUM_POSTS_PER_SIDE; i++) {
		post[index] = new Block(shaderIF, matl, newPost, ymin, (zmax - postWidth), postWidth, postHeight, postWidth);
		newPost -= delta;
		index++;
	}
	...
	for (int i = 0; i < NUM_POSTS_PER_SIDE; i++) {
		post[index] = new Block(shaderIF, matl, xmin, ymin, newPost, postWidth, postHeight, postWidth);
		newPost -= delta;
		index++;
	}
